...
export default class Mirror {
  ...
  bresenhamAlgorithm(x0, y0, x1, y1) {
    ...
    const reflectPoint = this.getRightCoordinate(...this.reflectPoint);
    ...
    while (true) {
      ...
      if (reflectX === newX0) {
        newX02 = newX0;
      } else if (reflectX > newX0) {
        newX02 = reflectX + (reflectX - newX0);
      } else {
        newX02 = reflectX - (newX0 - reflectX);
      }
      newY02 = newY0;

      this.drawPixel(newX0, newY0);
      this.drawPixel(newX02, newY02);
    }
  }
  ...
  init() {
    ...
    this.canvasBackground.addEventListener('contextmenu', Mirror.canselDefaultRightClich.bind(this));

    // Custom cursor
    const canvasWrapper = document.querySelector('#canvas__wrapper');
    canvasWrapper.style.cursor = 'url(\'https://www.piskelapp.com/p/img/cursors/vertical-mirror-pen.png\') 0 16, auto';
  }
}
